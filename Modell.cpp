...
Modell::Modell() : vao(0), vbo(0), vertexCount(0), hasTexCoords(false) {
...
void Modell::loadDataWithTexCoords(const float* float_array, int float_array_size, int stride) {
    hasTexCoords = true;
    vertexCount = float_array_size / stride;
...
bool Modell::loadOBJ(const std::string& path) {
    ...
    bool hasTextures = !attrib.texcoords.empty();
    hasTexCoords = hasTextures;
    ...
    for (const auto& sh : shapes) {
        for (const auto& idx : sh.mesh.indices) {
            ...
            if (hasTextures && idx.texcoord_index >= 0) {
                ...
                verts.push_back(attrib.texcoords[ti + 1]);
            } else if (hasTextures) {
    ...
    if (hasTextures) {
        loadDataWithTexCoords(verts.data(), (int)verts.size(), 8);
    } else {
